const REPO_BASE = 'https://saberbadri24-tech.github.io/city-of-eterniuty/';

async function boot() {
  const root = document.querySelector<HTMLDivElement>('#app');
  if (!root) throw new Error('App root not found');
  if (window.location.hash === '#admin') {
    const { renderAdmin } = await import('./admin');
    await renderAdmin(root);
    return;
  }
  const html = await fetch('./ui/index.html?ts=' + Date.now()).then((r) => {
    if (!r.ok) throw new Error('ui_' + r.status);
    return r.text();
  });
  const parsed = new DOMParser().parseFromString(html, 'text/html');
  document.documentElement.lang = parsed.documentElement.lang || 'fa';
  document.documentElement.dir = parsed.documentElement.dir || 'rtl';
  document.head.innerHTML = parsed.head.innerHTML;
  const base = document.createElement('base');
  base.href = REPO_BASE + 'ui/';
  document.head.prepend(base);
  document.body.innerHTML = parsed.body.innerHTML;
  Array.from(parsed.body.querySelectorAll('script')).forEach((source) => {
    if (!source.src) return;
    const script = document.createElement('script');
    script.src = source.src;
    script.defer = true;
    document.body.appendChild(script);
  });
  const sw = document.createElement('script');
  sw.textContent = "if ('serviceWorker' in navigator) navigator.serviceWorker.register('sw.js').catch(() => {});";
  document.body.appendChild(sw);
}

boot().catch((error) => {
  const root = document.querySelector<HTMLDivElement>('#app');
  if (root) root.innerHTML = '<main style="padding:40px;font-family:Arial;text-align:center">ANIL X is loading the experience. Please refresh once.</main>';
  console.error(error);
});
